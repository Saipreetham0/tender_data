...
const Navbar = () => {
  ...
  const handleSettingsClick = () => {
    setShowUserMenu(false);
    // You can add a settings page route here when available
            ...
            {navigationLinks.map((link) => {
              ...
              return (
                <button
                  key={link.name}
                  onClick={() => handleNavigation(link.href)}
                  className={`flex items-center space-x-3 px-2 py-3 rounded-lg transition-colors duration-150 w-full text-left ${
                    isActive
                      ? "bg-blue-50 text-blue-600 border border-blue-200"
