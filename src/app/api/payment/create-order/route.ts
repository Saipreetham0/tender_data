...
// src/app/api/payment/create-order/route.ts - UPDATED
import { NextResponse } from 'next/server';
import { RazorpayPaymentService } from '@/lib/razorpay-payment';

export async function POST(request: Request) {
  try {
    const { planId, subscriptionType, userEmail, userId } = await request.json();
    ...
    const result = await RazorpayPaymentService.createPaymentOrder({
      userEmail,
      planId,
      subscriptionType,
      userId
