// src/app/api/tenders/nuzvidu/route-optimized.ts - Optimized version using cached data
...
export async function GET(request: Request) {
  // Apply rate limiting
  const rateLimitResult = await rateLimitCheck(request, 'nuzvidu-tenders');
  if (rateLimitResult) {
  ...
  try {
    ...
    console.log("🚀 Nuzvidu API: Fetching cached data...");

    // Get cached data - this is MUCH faster than scraping
    const cachedData = await getCachedTenderData('nuzvidu');
    ...
    console.error("Error in Nuzvidu API route:", error);
    return NextResponse.json({
      success: false,
