...
// function fixRelativeUrl(url: string, baseUrl: string): string {
...
// function handleScrapingError(error: any, source: string): void {
//   console.error(`Scraping error from ${source}:`, error);
//   // You can add additional error handling here, like sending to a monitoring service
// }
...
async function scrapeSrikakulamTenders(): Promise<{ tenders: Tender[], totalPages: number, totalTenders: number }> {
  let browser = null;
  ...
  try {
    ...
    console.log(`Found ${totalPages} pages with approximately ${totalTenders} tenders`);
    ...
    for (let currentPage = 2; currentPage <= Math.min(totalPages, 43); currentPage++) {
      try {
...
export async function GET() {
  try {
    console.log("Starting tender fetch for RGUKT Srikakulam...");
    const { tenders, totalPages, totalTenders } = await scrapeSrikakulamTenders();
